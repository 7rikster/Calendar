...
View the deployed app here: [calendar-eta-lyart.vercel.app](https://calendar-eta-lyart.vercel.app/)
...
- **Decoupled Grid Slotting Mechanism**: Calculating the vertical stack-order of multi-day event bars _per day_ notoriously leads to broken "zig-zag" visualizations. We mitigated this by mathematically resolving an event's `Array Slot` locally for the _entire slice boundary_ globally from the Grid controller itself before distributing `paddedSlot` proxies to the generic Cells.
